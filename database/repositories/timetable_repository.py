...
class TimetableRepository(BaseRepository):
    ...
    # /     /     >---- نجيب حصة مع تفاصيلها (المقرر، الأستاذ، القاعة)
    ...
    # /     /     >---- نجيب الحصص حسب الشروط (مع التفاصيل)
    ...
    # /     /     >---- نجيب الحصص حسب الشروط (بدون الأوقات)
    ...
    # /     /     >---- نفحص هل القاعة ضربانة في نفس اليوم والفترة
    ...
    # /     /     >---- نحول وقت HH:MM لدقائق (يرجع None لو مش صالح)
    @staticmethod
    ...
    # /     /     >---- نفحص تداخل الأوقات (نهاية الساعة 12:00 ما تشغلش 12:00)
    @staticmethod
    ...
    # /     /     >---- نجيب القاعات أو الأساتذة مع حالتهم (متاح/مشغول)
    def get_available_resources(self, resource: str, day: str, semester: int,
        ...
        """
        # /     /     >---- نختار الجدول الأساسي حسب النوع (قاعة أو أستاذ)
        if resource == 'room':
        ...
        busy: Dict[Any, bool] = {}
        for b in bookings:
            # /     /     >---- نتجاهل الحصة الحالية عند التعديل
            if exclude_id and b['id'] == exclude_id:
                continue
            conflicted = False
            # /     /     >---- بالمشغل الزمني إذا الأوقات موجودة، وإلا بالفترات
        ...
        # /     /     >---- نعلّم كل مورد: 0 مشغول، 1 متاح
        for res in resources:
            res['is_available'] = 0 if busy.get(res['id']) else 1
        return resources

    # /     /     >---- نجيب بس الموارد المتاحة كاملةً
    ...
    # /     /     >---- نجيب الحصص المتعارضة مع مورد معين (تحذيرات فقط)
    def get_conflicting_entries(self, fk_col: str, fk_id: int, day: str,
        ...
        for b in rows:
            ...
            if b[fk_col] != fk_id:
                continue
            # /     /     >---- نفحص التعارض بالأوقات أو بالفترات
            if start_time and end_time and b['start_time'] and b['end_time']:
    ...
    # /     /     >---- بيانات جدول قسم معين (فصل/نسخة اختيارية)
    def get_department_view_data(self, dept_id: int, semester: int = None,
        ...
        if version_id is not None:
            # /     /     >---- مع خيار إظهار الأسطر القديمة بدون نسخة
            if include_legacy:
    ...
    # /     /     >---- مقررات القسم فقط (ما نعرضش القائمة الكاملة في النموذج)
    def list_department_courses(self, dept_id) -> List[Dict[str, Any]]:
        ...
        # /     /     >---- بدون قسم نرجع كل المقررات
        if not dept_id:
            return self.list_all_courses()
        # /     /     >---- اتحاد: المقررات المرتبطة + الرئيسية + المستعملة في الجدول
        return [dict(r) for r in self.db.execute(
    ...
    # /     /     >---- الأستاذ والمقرر الخاصين بحصة معينة
